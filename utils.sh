...
get_prebuilts() {
	...
	rv_cli_url=$(gh_req "https://api.github.com/repos/E85Addict/revanced-cli/releases/latest" - | json_get 'browser_download_url') || return 1
...
req() { _req "$1" "$2" "User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:110.0) Gecko/20100101 Firefox/110.0"; }
	...
	if [ ! -f "$stock_apk" ]; then
		if [ "$dl_from" = apkmirror ]; then
			...
			if ! dl_apkmonk "$apkmonk_resp" "$version" "$stock_apk"; then
				...
				declare -r dl_url=$"https://www.apkmonk.com/app/$pkg_name/#previous"
			fi
		fi
	fi
    if [ "$dl_from" = apkmirror ]; then
